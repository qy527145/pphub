...
3. **房间口令**：多台设备输入同一口令互连成全网状 Mesh（数据全网状 P2P；屏幕/语音在大房间自动改走服务器扇出，见下）。
...
共享、观看端切换画面与声音开关。**大房间（默认超过 8 人）自动改走服务器扇出**：
发送端只编码一次、用群密钥加密一次、上行一份，服务器复制密文给所有观众（只转发
密文、看不到明文），于是屏幕/语音能发给几十人而不压垮上行；此路径同样需要
WebCodecs（安全上下文）。

**语音对讲**：侧边栏「按住说话」实时对讲。小房间把麦克风轨直传各直连对端；
大房间同屏幕一样走服务器扇出（Opus 自编码 + 群密钥加密），全房间可达。
...
默认 32）。仍可通过 `PPHUB_STUN_URLS` / `PPHUB_TURN_URLS` / `PPHUB_TURN_SECRET`
