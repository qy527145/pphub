...
|---|---|---|
| https 端 → http 端 | ✅ 原生媒体轨，正常（含音频） | ❌ 对端解不了码，界面给出原因，不留黑屏条目 |
| http 端 → https 端 | ❌ 本端无 `getDisplayMedia`，按钮禁用并说明原因 | ❌ 同左，连采集都发不起 |

> ⚠️ **已知缺口**：走中继时，发起端的可达性预检只检查**本端能否编码**，对端的解码能力
> 并未在协议里通告。因此 https 端向 http 端发起中继共享时，预检会误判为「可达」并进入
> 「共享中」状态，实际对面收不到画面。兜底在接收端——它会拒绝并显示原因，**不会**挂出
> 永远黑屏的条目，所以不至于让人干等，但发起端的状态显示是不准的。
> 要彻底解决需要在 hello/名片消息里通告 `screenDecode` 能力。此行为由
> `e2e-mixed-context.mjs` 固定，修复时记得同步更新该断言与本表。
